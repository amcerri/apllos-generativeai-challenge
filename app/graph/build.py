"""
LangGraph graph builder (nodes, edges, state, and reducers).

Overview
  Defines the stateful multi-agent graph using LangGraph, with routing and
  pipelines for analytics (plan → exec → normalize), knowledge (retrieve → rank
  → answer), commerce (process_doc → extract_llm → summarize), and triage.
  Provides safe fallbacks when optional dependencies are missing.

Design
  - Optional imports with graceful degradation; stub returns when LangGraph is
    unavailable at import time.
  - Typed state via `TypedDict` and per-key reducers to support fan‑out.
  - Human‑in‑the‑loop interrupt helpers for SQL approval.

Integration
  - Consumed by `app.graph.assistant.get_assistant(...)` to compile/cache the
    graph instance used by the API and Studio.
